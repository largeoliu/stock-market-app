...
class StockAPI {
  ...
  /**
   * 解析日期字符串，兼容iOS
   * @param {string|number|Date} dateInput - 日期字符串、时间戳或日期对象
   * @returns {Date} 日期对象
   */
  parseDate(dateInput) {
    ...
    // 如果是数字（时间戳），直接用于创建Date
    if (typeof dateInput === 'number') return new Date(dateInput)
    
    // 处理字符串
    let dateString = String(dateInput)
    
    // 将 "yyyy-MM-dd HH:mm:ss" 格式转换为 "yyyy-MM-ddTHH:mm:ss"（iOS支持）
  ...
  formatFavoritesData(rawData) {
    ...
    const formattedFavorites = rawData.favorites.map(item => ({
      ...
      timestamp: this.parseDate(item.created_at).getTime(), // 转换为时间戳便于排序
    ...
    try {
      ...
      return {
        ...
        favorites: localFavorites.map(stock => ({
          ...
          created_at: this.parseDate(stock.timestamp || Date.now()).toISOString()
        })),
        syncResult: {
          error: error.message,
